...
Widget build(BuildContext context) {
              ...
              child: Column(
                children: [
                  ...
                  Row(
                    ...
                    children: (resultData.isNotEmpty && resultData[0]['result_number'] != null
                      ? resultData[0]['result_number'].toString().split('') 
                      : prizeNumbers).map((number) {
                      return Padding(
                  ...
                  PrizeRow('รางวัลที่ 2', resultData.isNotEmpty && resultData[1]['result_number'] != null 
                    ? resultData[1]['result_number'].toString() 
                    : 'x x x x x x', '50,000 บาท'),
                  PrizeRow('รางวัลที่ 3', resultData.isNotEmpty && resultData[2]['result_number'] != null 
                    ? resultData[2]['result_number'].toString() 
                    : 'x x x x x x', '30,000 บาท'),
                  PrizeRow('รางวัลที่ 4', resultData.isNotEmpty && resultData[3]['result_number'] != null 
                    ? resultData[3]['result_number'].toString() 
                    : 'x x x x x x', '10,000 บาท'),
                  PrizeRow('รางวัลที่ 5', resultData.isNotEmpty && resultData[4]['result_number'] != null 
                    ? resultData[4]['result_number'].toString() 
                    : 'x x x x x x', '5,000 บาท'),
                ],
              ),
            ),
          ),
    ...
    itemBuilder: (context, index) {
      ...
      if (resultData.isNotEmpty) {
        ...
        for (int i = 0; i < resultData.length; i++) {
          if (resultData[i]['result_number'] == lottoItem['lotto_number']) {
            switch (i) {
              case 0:
              ...
              ElevatedButton(
                onPressed: hasMatchingResult
                  ? () async {
                      ...
                      dv.log('ถูกรางวัล: $matchedPrizeCategory จำนวนเงิน: $matchedPrizeAmount');

                      // Send POST request to the API to add money
                      var response = await http.post(
                        Uri.parse('https://node-api-lotto.vercel.app/wallet/add_money'),
                        headers: {"Content-Type": "application/json"},
                        body: jsonEncode({
                          "userId": widget.idx, // Assuming widget.idx is the user ID
                          "amount": matchedPrizeAmount
                        }),
                      );

                      var responseBody = json.decode(response.body);
                      dv.log('response:'+responseBody['message']);
                      if (response.statusCode == 200 || response.statusCode == 201) {
                        // Check if the response body indicates success
                        if (responseBody['message'] == 'Deposit successfully') {
                          dv.log('เพิ่มเงินสำเร็จ: ${response.body}');
                          // Call deleteLotto function after successful "ถูกรางวัล"
                          await deleteLotto(lottoItem['lotto_id']);
                        } else {
                          dv.log('ข้อผิดพลาดในการเพิ่มเงิน: ${response.body}');
                        }
                      } else {
                        dv.log('ข้อผิดพลาดในการเพิ่มเงิน: ${response.statusCode} ${response.body}');
                      }
                    }
                  : null, // Disable button if not a winning number
              // Disable button if not a winning number
                style: ElevatedButton.styleFrom(
                  backgroundColor: hasMatchingResult ? Colors.green : Colors.grey, // Set green color for "ถูกรางวัล", grey otherwise
                  padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 12.0), // Adjust button padding if needed
                ),
                child: Text(
                  resultData.isEmpty
                      ? 'รอออกรางวัล' // If results are not available yet
                      : (hasMatchingResult ? 'ถูกรางวัลที่ $matchedPrizeCategory' : 'ไม่ถูกรางวัล'), // Show prize category and amount if a match, otherwise "ไม่ถูกรางวัล"
                  style: const TextStyle(
  ...
  Future<void> showResult() async {
    try {
      // Include the user ID (widget.idx) in the API call URL
      var response = await http.get(Uri.parse(
          'https://node-api-lotto.vercel.app/result/'));
